...
<div class="container mt-5">
    ...
    <table class="table fs-5">
        ...
        @foreach($records as $record)
            <tr class="deleted-record">
                ...
                <td>
                    <form action="{{ route('restore_user', $record->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                ...
                <td>
                    <form action="{{ route('deletedestroy_user', $record->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
    ...
    @endif
    <a href="{{ route('admin.user') }}" class="mt-3 d-inline-block">Вернуться назад</a>
</div>
</body>
</html>
